...
from ..tests._datasets import R_data, py_data
from ..tests._rutility import (R_attgt_to_pandas, r_agg, r_agg_not_overall,
                               r_agg_overall, r_attgt)
...
@pytest.mark.parametrize(
    "data_name,anticipation,base_period," "control_group,est_method,weights_name", args
)
def test_ATTgt_with_Rdid(
    ...
    for agg_type in ["dynamic", "simple", "calendar", "group"]:
        ...
        py_agg_overall_result = att_gt.aggregate(
            python_agg_types[agg_type], overall=True
        )
        ...
        if agg_type in [
            ...
            "simple",
            "calendar",
        ]:  # group_overall likely a bug in R
            assert np.allclose(
